...
class MonitoringObject(MonitoringObjectSerializer):
    def get2(self, value=None, field_name=None, depth=0):
    ...
    def get(self, user, scope, value=None, field_name=None, depth=0):
        """
        ...
        try:
            ...
            if self._object_type == "sites_group":
                pass

            if self._object_type == "site":
                # q = q.outerjoin(TBaseSites, Model.id_base_site == TBaseSites.id_base_site)
                if scope == 2:
                    q = q.filter(
                        or_(
                    ...
                    #     )
                    # )
            if self._object_type == "visit":
                if scope == 2:
                    q = q.filter(
                        Model.observers == user.id_role,
                    )
                elif scope == 1:
                    q = q.filter(
                        Model.observers == user.id_role,
            ...
            if self._object_type == "observation":
                pass

            if self._object_type == "observation_detail":
                pass

            self._model = q.filter(getattr(Model, field_name) == value).one()
